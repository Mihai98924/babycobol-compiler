...
DIVIDE: D WS* I WS* V WS* I WS* D WS* E -> pushMode(DIV_REP);
...
MULTIPLY: M WS* U WS* L WS* T WS* I WS* P WS* L WS* Y -> pushMode(MUL_REP);
...
SUBTRACT: S WS* U WS* B WS* T WS* R WS* A WS* C WS* T -> pushMode(SUB_REP);
...
NOT_TO: ( ~[T] | T ~[O] )+;
ATOMIC_TO: NOT_TO | NUMERICLITERAL | ZERO | INTEGERLITERAL | STRINGLITERAL;
ADD_REPRESENTATION: WS* (ATOMIC_TO WS*)+ TO -> popMode;

mode SUB_REP;

NOT_FRON: ( ~[F] | F ~[R] | F R ~[O] | F R O ~[M] )+;
ATOMIC_FROM: NOT_FRON | NUMERICLITERAL | ZERO | INTEGERLITERAL | STRINGLITERAL;
SUB_REPRESENTATION: WS* (ATOMIC_FROM WS*)+ FROM -> popMode;

mode MUL_REP;

NOT_BY: ( ~[B] | B ~[Y] )+;
ATOMIC_BY: NOT_BY | NUMERICLITERAL | ZERO | INTEGERLITERAL | STRINGLITERAL;
MUL_REPRESENTATION: WS* (ATOMIC_BY WS*)+ BY -> popMode;

mode DIV_REP;

NOT_INTO: ( ~[I] | I ~[N] | I N ~[T] | I N T ~[O] )+;
ATOMIC_INTO: NOT_INTO | NUMERICLITERAL | ZERO | INTEGERLITERAL | STRINGLITERAL;
DIV_REPRESENTATION: WS* (ATOMIC_INTO WS*)+ INTO -> popMode;
